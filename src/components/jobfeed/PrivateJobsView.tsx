...
export default function PrivateJobsView({ onBack }: Props) {
  ...
  useEffect(() => {
    const fetchPrivateJobs = async () => {
      try {
        setLoading(true);
        const { data, error } = await supabase.from('jobs').select('*').eq('type', 'private');
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          {processedJobs.length > 0 ? (
            ...
            <Text style={[styles.emptyText, { color: theme.subText }]}>{t.no_private_jobs_found || 'კერძო შეკვეთები არ მოიძებნა'}</Text>
          )}
        </ScrollView>
      )}
    </View>
