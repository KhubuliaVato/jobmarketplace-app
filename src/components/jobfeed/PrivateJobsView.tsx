...
export default function PrivateJobsView({ onBack }: Props) {
  ...
  useEffect(() => {
    const fetchPrivateJobs = async () => {
      try {
        setLoading(true);
        const { data, error } = await supabase.from('jobs').select('*').eq('type', 'private').or('status.is.null,status.eq.active');
        ...
        console.log('კერძო შეკვეთების ჩატვირთვის ხარვეზი:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.bg }]}>
      <View style={styles.headerRow}>
        ...
        <Text style={[styles.title, { color: theme.text }]}>{t.private_jobs_title || 'კერძო შეკვეთები 🛠️'}</Text>
      </View>
      ...
      <View style={[styles.searchContainer, { backgroundColor: theme.searchBg }]}>
        ...
        <TextInput
          ...
          placeholder={t.search_private_job_placeholder || "მოძებნე კერძო შეკვეთა..."}
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          {processedJobs.length > 0 ? (
            ...
            <EmptyState
              ...
              title={t.no_private_jobs_found || 'კერძო შეკვეთები არ მოიძებნა'}
              subtitle="ამ კატეგორიაში ჯერ განცხადება არ არის"
            />
          )}
        </ScrollView>
      )}
